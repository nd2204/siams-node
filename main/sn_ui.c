...
// palette.
static const size_t kFrameBufferSize =
    LCD_H_RES * LCD_V_RES / 8 + LVGL_PALETTE_SIZE;
...
void init_ssd1306_panel() {
  ...
  ESP_ERROR_CHECK(esp_lcd_new_panel_io_i2c(
      sn_devmgr_get_i2c_master_bus_handle(), &io_config, &g_ioHandle));
  ...
  ESP_ERROR_CHECK(
      esp_lcd_new_panel_ssd1306(g_ioHandle, &panel_config, &g_panelHandle));
...
void init_lgvl() {
  ...
  oled_back_buffer = heap_caps_calloc(1, kFrameBufferSize,
                                      MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  assert(oled_back_buffer);

  // LVGL9 suooprt new monochromatic format.
  lv_display_set_color_format(g_display, LV_COLOR_FORMAT_I1);
  // initialize LVGL draw buffers
  lv_display_set_buffers(g_display, oled_back_buffer, NULL, kFrameBufferSize,
  ...
  esp_lcd_panel_io_register_event_callbacks(g_ioHandle, &cbs, g_display);
}
...
void lvgl_task() {
  ...
  _lock_acquire(&lvgl_api_lock);
  lv_obj_t *scr = lv_display_get_screen_active(g_display);
  ...
  lv_obj_set_width(label, lv_display_get_horizontal_resolution(g_display));
