...
static void mqtt_event_handler(void *args, esp_event_base_t base, int32_t eid, void *edata) {
  ...
  switch ((esp_mqtt_event_id_t)eid) {
    case MQTT_EVENT_CONNECTED: {
      ...
      cJSON *root = create_register_payload(ctx->deviceId);
...
esp_err_t mqtt_registration_run(const char *broker_uri) {
  char mqtt_user[64];
  char mqtt_pass[64];
  ...
  esp_mqtt_client_register_event(reg_client, ESP_EVENT_ANY_ID, mqtt_event_handler, NULL);
  esp_mqtt_client_start(reg_client);

  // Wait until ack received or timeout (15s)
  int retry = 0;
  while (!got_ack && retry++ < 60) {
